...
public class RCC_Editor : Editor {
	...
	static void CreateBehavior(){
		...
		if(!Selection.activeGameObject.GetComponentInParent<RCC_CarControllerV3>()){
			...
			if (fixPivot) {
				...
				selectedVehicle.GetComponent<Rigidbody>().mass = 1400f;
	...
	private void WheelSettingsTab() {
		...
		EditorGUILayout.PropertyField(serializedObject.FindProperty("overrideWheelAll"), new GUIContent("Override All Wheels", "Override All Wheels."), false);

		if (carScript.overrideWheelAll)
			EditorGUILayout.HelpBox("Each wheel can be customized above by clicking their edit buttons. Such as power, steer, brake, and handbrake.", MessageType.Info);
	...
	private void DamageTab() {
		...
		EditorGUILayout.Space();
		EditorGUILayout.PropertyField(serializedObject.FindProperty("useDeformation"), new GUIContent("Use Deformation"), false);
		...
		if (carScript.useDeformation) {

			EditorGUILayout.PropertyField(serializedObject.FindProperty("damage"), new GUIContent("Deformation", "Deformation"), true);
